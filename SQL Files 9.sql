...
FROM (
  ...
  END AS match_status
FROM (select source, Transactionid, SUM(amount) AS amount
		from transactions
		where Source = 'accounting_system'  or Source = 'payment_sevice_provider'
		group by source, transactionid) AS s1 
 JOIN transactions AS s2 ON s1.transactionid = s2.transactionid
  AND s1.source != s2.source
) AS combined_data
#WHERE source1 = 'web_app' or   source1 = 'accounting_system'
GROUP BY source1;